...
export async function submitTravelBooking(formData: FormData) {
  ...
  // Persist the editable "auto-fill" profile so future forms can prefill from login.
...
export async function updateTravelBooking(referenceNo: string, formData: FormData) {
  const session = await auth();
  const submitterEmail = session?.user?.email?.toLowerCase();
  const submitterName = session?.user?.name ?? submitterEmail ?? "";
  if (!submitterEmail) throw new Error("Not signed in");
